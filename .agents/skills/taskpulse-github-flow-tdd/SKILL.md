...
description: Use when working on TaskPulse Agent tickets so Codex follows the repository's GitHub Flow and TDD process, starts from the assigned ticket in BE/.agents/tasks or FE/.agents/tasks, writes tests first, keeps PR scope single-purpose, and updates docs/contracts when interfaces change.
---
...
4. If the task changes an API or UI contract, read the relevant docs in `BE/docs/` or `FE/docs/`.
5. Work in this order:
...
6. Keep the change single-purpose and aligned to the ticket branch.
